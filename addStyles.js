const bodyClass = "bg-blue-300 bg-gradient-to-bl from-blue-900 bg-opacity-50";
const bdy = document.querySelector("body");
bodyClass.split(" ").forEach((cls) => bdy.classList.add(cls));

const calcBodyClass =
  "max-w-lg rounded overflow-hidden shadow-2xl bg-purple-800 bg-opacity-75 bg-gradient-to-tr from-white-800 flex";
const calcBody = document.querySelector(".calcBody");
calcBodyClass.split(" ").forEach((cls) => calcBody.classList.add(cls));

const outputClass =
  "flex flex-col w-3/4 h-12 justify-center items-end m-1 bg-gray-300 text-gray-900 font-semibold px-2 border border-black";
...
function addClasses(button) {
  const btnNumberClass = "hover:bg-green-900 border-green-500";
  const btnClearClass = "hover:bg-orange-900 border-orange-500";
  const btnOpClass = "hover:bg-blue-900 border-blue-500";
  const btnEqClass = "hover:bg-red-900 border-red-500";
  const btnDecClass = "hover:bg-yellow-900 border-yellow-500";
